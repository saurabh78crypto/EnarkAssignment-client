...
const Dashboard = () => {
 ...
 const fetchBooks = async () => {
    try {
      const response = await axios.get('https://enarkassignment-server.onrender.com/api/auth/books', {
        params: {
          page: currentPage + 1,
          search: searchTerm,
        },
...
useEffect(() => {
 ...
 const handleModalSubmit = async (updatedBook) => {
    ...
    try {
        const response = await fetch(`https://enarkassignment-server.onrender.com/api/auth/books/${editingBook._id}`, {
            method: 'PUT',
            headers: {
 ...
 const handleEditBook = (bookId) => {
   ...
   const handleDeleteBook = async (bookId) => {
    try {
        const response = await fetch(`https://enarkassignment-server.onrender.com/api/auth/books/${bookId}`, {
            method: 'DELETE',
        });
